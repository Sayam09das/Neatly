...
* **Notifications:** Persisted customer inbox rows use `Notification.recipientId` = session user id. Booking create/update/cancel and review create write a customer-safe inbox item after the mutation succeeds (failures are logged and do not roll back the mutation). Admin SSE remains admin-only. There is no customer WebSocket/SSE, push, email, or SMS delivery in this layer. Customers cannot POST system notifications. Other customers' notifications return not-found. `relatedHref` values outside `/dashboard` are omitted from customer responses.
