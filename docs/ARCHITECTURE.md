...
* **Authenticated API Client:** Same-origin `adminRequest()` sends the session cookie (`credentials: "same-origin"`, `cache: "no-store"`). Browser Admin GETs use `/api/v1/admin/*`; the Next.js BFF forwards the session as `x-session-token` to the HTTP API and never returns that token to JavaScript. Typed Admin clients live in `apps/web/src/lib/admin/`. HTTP 401 is unauthorized / session invalid and reuses the existing Admin login redirect. HTTP 403 is authenticated-but-forbidden and must not log the user out. There is no retry or token-refresh loop. Admin responses are `Cache-Control: no-store` and are never publicly cached.
