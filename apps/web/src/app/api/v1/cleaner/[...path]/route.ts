...
interface CleanerProxyContext {
  params: Promise<{ path: string[] }>;
}

export async function GET(
  request: Request,
  context: CleanerProxyContext,
): Promise<Response> {
  return proxyCleanerRequest(request, context, "GET");
}
...
export async function POST(
  ...
  return proxyCleanerRequest(request, context, "POST");
}
...
export async function PATCH(
  ...
  return proxyCleanerRequest(request, context, "PATCH");
}

async function proxyCleanerRequest(
  request: Request,
  context: CleanerProxyContext,
  ...
  try {
    ...
    if (segments.includes("admin") || segments.includes("customer")) {
    ...
    const target = `${origin}/api/v1/cleaner/${segments.join("/")}${search}`;
    const jar = await cookies();
    const sessionToken = jar.get(AUTH_SESSION_COOKIE_NAME)?.value;
    const headers = new Headers({
      accept: "application/json",
      "x-forwarded-for": getRequestIp(request),
